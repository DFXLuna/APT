...
namespace NarrativeReader{
	...
	public class Reader{
		...
		public bool readNextScene(out KeyValuePair<condition, NarrativeEvent> e){
			if(file.Peek() == -1){ 
				e = new KeyValuePair<condition, NarrativeEvent>();
				return false;
			}
			...
			e = new KeyValuePair<condition, NarrativeEvent>(c, parseScene(lines));
			return true;			
		}
